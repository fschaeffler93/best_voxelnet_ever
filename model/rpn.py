...
class MiddleAndRPN:
    def __init__(self, input, alpha=1.5, beta=1, sigma=3, training=True, decrease=False, minimize=False, name=''):
        ...
        with tf.variable_scope('MiddleAndRPN_' + name):
            ...
            temp_conv = tf.reshape(temp_conv, [-1, cfg.INPUT_HEIGHT, cfg.INPUT_WIDTH, 128//factor])

            # rpn
            # block1:
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (2, 2), (1, 1), temp_conv, training=self.training, name='conv4')
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv5')
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv6')
            if not self.decrease:
                temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv7')
            deconv1 = Deconv2D(128//factor, 256//factor, 3, (1, 1), (0, 0), temp_conv, training=self.training, name='deconv1')

            # block2:
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (2, 2), (1, 1), temp_conv, training=self.training, name='conv8')
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv9')
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv10')
            temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv11')
            if not self.decrease:
                temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv12')
                temp_conv = ConvMD(2, 128//factor, 128//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv13')
            deconv2 = Deconv2D(128//factor, 256//factor, 2, (2, 2), (0, 0), temp_conv, training=self.training, name='deconv2')

            # block3:
            temp_conv = ConvMD(2, 128//factor, 256//factor, 3, (2, 2), (1, 1), temp_conv, training=self.training, name='conv14')
            temp_conv = ConvMD(2, 256//factor, 256//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv15')
            temp_conv = ConvMD(2, 256//factor, 256//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv16')
            temp_conv = ConvMD(2, 256//factor, 256//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv17')
            if not self.decrease:
                temp_conv = ConvMD(2, 256//factor, 256//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv18')
                temp_conv = ConvMD(2, 256//factor, 256//factor, 3, (1, 1), (1, 1), temp_conv, training=self.training, name='conv19')
            deconv3 = Deconv2D(256//factor, 256//factor, 4, (4, 4), (0, 0), temp_conv, training=self.training, name='deconv3')

            # final:
            temp_conv = tf.concat([deconv3, deconv2, deconv1], -1)
            # Probability score map, scale = [None, 200/100, 176/120, 2]
            p_map = ConvMD(2, 768//factor, 2, 1, (1, 1), (0, 0), temp_conv, training=self.training, activation=False, bn=False, name='conv20')
            # Regression(residual) map, scale = [None, 200/100, 176/120, 14]
            r_map = ConvMD(2, 768//factor, 14, 1, (1, 1), (0, 0), temp_conv, training=self.training, activation=False, bn=False, name='conv21')
